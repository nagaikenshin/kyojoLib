...
class OptGroupElement extends FormElement {
	...
	OptGroupElement(String label, List<OptionElement> options) {
		this.attrs = [ label : label ]
		this.options = options
	}

	OptGroupElement(Map<String, String> attrs, List<OptionElement> options) {
		this.attrs = attrs
		this.options = options
	}
	...
	public static class Builder extends org.kyojo.plugin.html5.HtmlElementImpl.Builder<OptGroupElement> {
		...
		@Override
		OptGroupElement build() {
			OptGroupElement obj = super.build()
			obj.options = options
			return obj
		}
